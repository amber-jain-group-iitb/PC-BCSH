...
!! Potential
integer nquant
real*8 g_coup,epsilon,V_coup
real*8 V_exothermicity,V12,V23,V34,V45,V56,V67,V47,omg_B,gamma_B,temperature
real*8 V13,V14,V15,V16,V17,V24,V25
real*8 V26,V27,V35,V36,V37,V46,V57
real*8 V11,V22,V33,V44,V55,V66,V77,Q,Q1,Q2,Q3,Q4,Q5,Q6,Q7
real*8 beta,gamma_D,lambda_B,V_reorg,V_barrier,pt
real*8 s01,s02,x_cr
...
real*8,allocatable :: eta(:,:),v_d(:,:),v_d_old(:,:)
...
!gamma_B=omg_B**2/gamma_D
 ...
 allocate(pop(nquant,i),pop_surf(nquant,i),pop_amp(nquant,i))
  ...
  allocate(eta(nquant,nclass),v_d(nquant,nclass),v_d_old(nquant,nclass))
...
do i=1,N_traj
    traj_num=i
    V_coup=0.01d0;call setup_parameters
    call init_cond
    call evolve(1,5000)
...
!-----------------------------------------------------------------  
subroutine try_decoherence(dtc)
  implicit none
  integer i,j
  ...
  do i=1,nquant
  do j=1,nclass
  if(((V_k(state)-V_k(i))+(0.5d0*mass(1)*(sum(v**2)))).lt.0d0) then
  eta(i,j)=1d-40
  else
  eta(i,j)=sqrt(2d0*(V_k(state)-V_k(i))/(mass(1)*(sum(v**2)))+1)
  endif
  v_d(i,j)=eta(i,j)*v(j)
  enddo
  enddo

end subroutine try_decoherence
!--------------------------------------------------------------
...
subroutine evolve_classical(dt)
  ...
  do i=1,nquant
  v_d(i,:)=v_d_old(i,:)+force(i,:)*dtc/mass(1)
  enddo
...
!    mat_adiab(i,i)=mat_adiab(i,i)-iota/hbar*(V_k(i)-V_avg)
    if (i==state) then
     mat_adiab(i,i)=mat_adiab(i,i)+iota/hbar*mass(1)*dot_product(v,v)
    else
     mat_adiab(i,i)=mat_adiab(i,i)+iota/hbar*mass(1)*dot_product(v(:),v_d(i,:))
    endif
  enddo
...
subroutine collapse(dt,iflag_coll)
  ...
  integer :: i,j,k,active(nquant),active_state,count_act
  complex*16 R_state(nquant),NR_state(nquant)
  real*8 R_sum,NR_sum
...
R_state=0d0
 NR_state=0d0
do i=1,nquant
 if(dot_product(ck(:),v_d_old(i,:))*dot_product(ck(:),v_d(i,:)).lt.0d0) then
    iflag_coll=1
...
enddo
R_sum= sum(R_state*conjg(R_state))
NR_sum= sum(NR_state*conjg(NR_state))
!stop
!write(*,*) active,active_state

!stop
count_act=0

 do j=1,active_state
    if(active(j)==state) then
...
if(count_act==1) then
ci=R_state/dsqrt(1d0-NR_sum)
else
ci=NR_state/dsqrt(1d0-R_sum)
endif
...
!H_diab(i+nquant/2,j+nquant/2)=H_diab(i+nquant2,j+nquant/2)+H2*expec_sqr_q22(i,j)
H_diab(i+nquant/2,j+nquant/2)=H_diab(i+nquant/2,j+nquant/2)-H3*expec_q22(i,j)
 ...
 H_diab(i,j+nquant/2)=Hamil_site(1,2)*(expec12(i,j))
  ...
  delV_dels(i,j,:)=delV_dels(i,j,:)+(mass*(omg**2)*x)*expec11(i,j)-ck*expec_q11(i,j)
  delV_dels(i+nquant/2,j+nquant/2,:)=delV_dels(i+nquant/2,j+nquant/2,:)+(mass*(omg**2)*x)*expec22(i,j)-ck*expec_q22(i,j)
